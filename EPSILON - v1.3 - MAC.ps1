...
# Common folders used by Delegation Management
$global:TargetFolders = @("Inbox", "Calendar", "Contacts", "Tasks", "Notes", "Journal", "Drafts", "Sent Items")
...
function Show-Help {
    ...
    Write-Host "   - Delegation management"
    ...
    Write-Host "  7: Delegation Management"
    Write-Host ""
    Write-Host "DELEGATION MANAGEMENT:"
    Write-Host "  1: Update folder-level delegate permissions"
    Write-Host "  2: Review folder-level delegate permissions"
    Write-Host "  3: Review mailbox-level delegation"
    Write-Host "  4: Downgrade mailbox-level delegate to folder-level Reviewer"
    Write-Host ""
...
function Show-MainMenu {
    param (
        [string]$Title = 'Exchange Online Administration Menu (EPSILON v1.3)'
    )
    Clear-Host
    Write-Host "================ $Title ================"
    Write-Host "1: Mailbox Management"
...
function Show-Permissions {
    ...
    if ($permissions) {
        ...
        Write-Host "No permissions found or folder inaccessible." -ForegroundColor Yellow
    }
}

function Show-MailboxDelegation {
    ...
    if ($sendOnBehalf) {
        $sendOnBehalf | Format-Table -AutoSize
    } else {
        Write-Host "None assigned." -ForegroundColor Yellow
    }
}

function Update-FolderLevelDelegatePermissions {
    Clear-Host
    $mailbox = Read-Host "Enter the mailbox to update (e.g., user@domain.com)"
    $delegatesInput = Read-Host "Enter the delegate users for $mailbox (comma-separated)"
    $delegates = $delegatesInput -split "," | ForEach-Object { $_.Trim() } | Where-Object { $_ }

    foreach ($folder in $global:TargetFolders) {
        $folderPath = "${mailbox}:\$folder"

        Write-Host "`n--- BEFORE: $folder ---" -ForegroundColor DarkYellow
        Show-Permissions -FolderPath $folderPath

        foreach ($delegate in $delegates) {
            $existing = Get-MailboxFolderPermission -Identity $folderPath -User $delegate -ErrorAction SilentlyContinue
            if ($existing) {
                if ($existing.AccessRights -ne "Reviewer") {
                    Set-MailboxFolderPermission -Identity $folderPath -User $delegate -AccessRights Reviewer -ErrorAction SilentlyContinue
                }
            } else {
                Add-MailboxFolderPermission -Identity $folderPath -User $delegate -AccessRights Reviewer -ErrorAction SilentlyContinue
            }
        }

        Write-Host "`n--- AFTER: $folder ---" -ForegroundColor Green
        Show-Permissions -FolderPath $folderPath
    }

    Write-Host "`nPermissions updated for $mailbox." -ForegroundColor Green
    Read-Host "Press Enter to continue"
}

function Review-FolderLevelDelegatePermissions {
    Clear-Host
    $mailbox = Read-Host "Enter the mailbox to review (e.g., user@domain.com)"

    foreach ($folder in $global:TargetFolders) {
        $folderPath = "${mailbox}:\$folder"
        Write-Host "`n--- Permissions for $folderPath ---" -ForegroundColor Cyan
        Show-Permissions -FolderPath $folderPath
    }
...
function Review-MailboxLevelDelegation {
    Clear-Host
    $mailbox = Read-Host "Enter the mailbox to review mailbox-level delegation (e.g., user@domain.com)"
    Show-MailboxDelegation -Mailbox $mailbox
    Read-Host "Press Enter to continue"
}

function Downgrade-MailboxLevelDelegate {
    Clear-Host
    $mailbox = Read-Host "Enter the mailbox to downgrade (e.g., user@domain.com)"
    $delegate = Read-Host "Enter the delegate to downgrade (e.g., assistant@domain.com)"

    Write-Host "`nRemoving Full Access from $delegate..." -ForegroundColor Yellow
    Remove-MailboxPermission -Identity $mailbox -User $delegate -AccessRights FullAccess -Confirm:$false -ErrorAction SilentlyContinue

    foreach ($folder in $global:TargetFolders) {
        $folderPath = "${mailbox}:\$folder"
        Write-Host "`nUpdating $folderPath to Reviewer for $delegate..." -ForegroundColor Cyan
    ...
    Write-Host "`n$delegate has been downgraded to Reviewer access only." -ForegroundColor Green
    Read-Host "Press Enter to continue"
}
                ...
                switch ($input) {
                    ...
                    '7' {
                        do {
                            ...
                            if ($subInput -eq 'B') { break }
                            switch ($subInput) {
                                '1' { Update-FolderLevelDelegatePermissions }
                                '2' { Review-FolderLevelDelegatePermissions }
                                '3' { Review-MailboxLevelDelegation }
                                '4' { Downgrade-MailboxLevelDelegate }
                            }
                            if ($global:DeltaQuit) { break }
                        } while ($true)
                    }
